...
# чтобы телефоны дергал и почты и сам на почту коммерческое отправлял (Круто? да?)
# описание более корректное,
# чтобы проверял актиный ли сайт (домен привязан не привязан)
...
# так же , чтобы не в "база" добавлялись , а в какую то категорию ("горячие" например , чтобы не отвалились через 10 дней )
# А ещё прикольно было бы сделать. Когда не знаешь кого собрать система предлагала варианты запросов , из списка дорогих
# кликов или из ниш или ещё откуда нибудь. возможно кому то будет полезно , а может нет )
#
...
def search_rubbish(list_url):
    result = []
    except_list = (
    'blog', 'google', 'avito', 'ozon', 'yandex', 'lenta', 'tiu', 'drom.ru', 'vk.com', '2gis.ru', 'zoon.ru','instagram')
    list_url = list_url
    flag = True
    for url in list_url:
